...
BEGIN
    BEGIN TRY
        ...
        IF EXISTS(
            SELECT 1
            FROM court_booking ck WITH (UPDLOCK, HOLDLOCK) -- sử dụng updlock để chặn lỗi phantom
                JOIN booking_slots bs WITH (UPDLOCK, HOLDLOCK) ON ck.id = bs.court_booking_id
