...
-- =============================================
-- T2: THU NGÂN TÍNH HÓA ĐƠN 
-- Demo: Unrepeatable Read Error
-- Vai trò: Thu ngân
-- Hành động: Đọc discount_policy 2 lần trong cùng 1 transaction, kết quả khác nhau
-- =============================================
...
BEGIN
    ...
    BEGIN
        ...
        WAITFOR DELAY '00:00:10'; -- sau khi đọc discount_policy lần 1, chờ T1 cập nhật dữ liệu
...
END
GO
-- demo lỗi unrepeatable read từ T2


SET TRANSACTION ISOLATION LEVEL REPEATABLE READ; --Nâng cấp mức độ cô lập để khắc phục lỗi unrepeatable read
BEGIN TRANSACTION; -- Bắt đầu transaction để giữ khóa Shared Lock trong suốt quá trình

SELECT 
    id AS DiscountPolicyId,
    [name] AS PolicyName,
...
EXEC SP_CalculateInvoice 
    @DiscountPolicyIds = '[1]',
    @CourtBookingId = 1,
    @PaymentMethod = N'Tiền mặt';

COMMIT TRANSACTION; -- Kết thúc transaction, giải phóng khóa
GO
