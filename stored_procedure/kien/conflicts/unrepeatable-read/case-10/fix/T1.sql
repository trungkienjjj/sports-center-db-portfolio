...
/* ====================================================
   T1: HỆ THỐNG TÍNH LƯƠNG (DEMO CONFLICT 2)
   Mục tiêu: Demo lỗi Unrepeatable Read (Dữ liệu cũ)
   Cập nhật: Đồng bộ cấu trúc với SP gốc (Snapshot toàn bộ kèm tính phạt)
   ==================================================== */
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        -- [FIX]: Nâng mức cô lập lên REPEATABLE READ
        -- Để giữ Shared Lock trên bảng Employee đến hết giao dịch
        BEGIN TRANSACTION;
        ...
        PRINT N'1. [Demo T1] Đã Snapshot dữ liệu (Lưu mức lương CŨ vào bảng tạm).';

        -- B2: DELAY
        PRINT N'2. [Demo T1] Hệ thống đang xử lý (Delay 10s)...';
        ...
        PRINT N'3. [Demo T1] Đã tính lương xong (Đúng với thực tế).';
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
...
/* ====================================================
   SCRIPT CHẠY DEMO (GỌI HÀM)
   ==================================================== */
-- 1. Reset dữ liệu về trạng thái ban đầu (Ví dụ lương 10tr)
UPDATE employee SET base_salary = 10000000 WHERE id = 1; 
-- Xóa lịch sử lương tháng này nếu có
DELETE FROM salary_history WHERE [month] = 12 AND [year] = 2025;

-- 2. Chạy T1 (Tính lương tháng 12/2025)
