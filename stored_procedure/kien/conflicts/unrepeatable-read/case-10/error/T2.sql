 ...
 * ====================================================================
 */
CREATE OR ALTER PROCEDURE sp_manager_update_employee_profile
...
PRINT N'=== ĐÃ TẠO STORED PROCEDURE: sp_manager_update_employee_profile ===';
GO

/* ====================================================
   PHẦN SCRIPT DEMO (GỌI HÀM)
   ==================================================== */
UPDATE employee SET base_salary = 10000000 WHERE id = 1; -- Lương gốc 10tr

DELETE FROM salary_history WHERE employee_id = 1 AND [month] = 12 AND [year] = 2025; --xóa bảng lương tháng 12 nếu đã có

PRINT N'=== T2: CẬP NHẬT LƯƠNG (CHEN NGANG) ===';

-- 1. Lấy ID Manager (Giả lập user đang đăng nhập)
DECLARE @MgrID UNIQUEIDENTIFIER = 'A23A9E53-C2F0-43B3-8807-DD16C7D4C400'; 

-- 2. Gọi SP Gốc để update lương
-- Hành động này sẽ xảy ra TRONG KHI T1 đang Delay
EXEC sp_manager_update_employee_profile
    @ManagerUserID = @MgrID,
    @EmployeeID = 1,        -- Nhân viên cần sửa
    @BaseSalary = 50000000; -- Mức lương mới (Gây lệch số liệu bên T1)
GO

-- Xem lịch sử lương vừa tính -> Vẫn là 10.000.000 (BỊ CŨ)
SELECT employee_id, base_salary, gross_pay FROM salary_history 
WHERE employee_id = 1 AND [month]=12;

-- Xem lương thực tế trong hồ sơ -> Là 50.000.000 (MỚI)
SELECT id, base_salary FROM employee WHERE id = 1;