...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            FROM branch_service bs WITH (UPDLOCK)
                JOIN service s ON bs.service_id = s.id
            WHERE bs.id = @branch_service_id;

            PRINT 'Current stock: ' + CAST(@current_stock AS NVARCHAR(10));

            PRINT 'Waiting for 10 seconds...';
            -- Wait to allow T2 (manager) to update the same service concurrently.
            -- This helps reproduce dirty-read / lost-update scenarios on stock.
            ...
            PRINT 'Updated current stock: ' + CAST(@current_stock - @quantity AS NVARCHAR(10));
...
GO
-- ===================================================================
-- Test ERR04 (dirty read) với Nước Revive (branch_service_id = 1)
-- Session T1 - lễ tân
SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
BEGIN TRAN;
EXEC sp_create_service_booking
    @court_booking_id = 1,
    @employee_id = 1,
    @items = N'[{"branch_service_id":5,"quantity":3,"start_time":"2025-12-05T10:00:00","end_time":"2025-12-05T11:00:00","by_month":0,"employee_id":1}]';
COMMIT;