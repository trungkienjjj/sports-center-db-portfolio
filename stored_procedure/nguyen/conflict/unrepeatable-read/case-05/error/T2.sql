...
/*
 * Manager cấu hình tham số cho chi nhánh của mình (USP_MANAGER_CONFIG_BRANCH)
 * 
 * Chức năng:
 ...
 *
 * Ví dụ:
 *   EXEC USP_MANAGER_CONFIG_BRANCH
 *       @ManagerUserID = 'GUID-MANAGER',
 *       @ShiftPay = 120000,
 *       @CancelFeeWithin24h = 0.60;
 * ====================================================================
 */
CREATE OR ALTER PROCEDURE USP_MANAGER_CONFIG_BRANCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        PRINT 'OLD LOYALTY POINT RATE: ' + CAST(@LoyaltyPointRate AS NVARCHAR(10));
        PRINT 'OLD CANCEL FEE BEFORE 24H: ' + CAST(@CancelFeeBefore24h AS NVARCHAR(10));
        PRINT 'OLD CANCEL FEE WITHIN 24H: ' + CAST(@CancelFeeWithin24h AS NVARCHAR(10));
        ...
        PRINT 'NEW LOYALTY POINT RATE: ' + CAST(@LoyaltyPointRate AS NVARCHAR(10));
        PRINT 'NEW CANCEL FEE BEFORE 24H: ' + CAST(@CancelFeeBefore24h AS NVARCHAR(10));
        PRINT 'NEW CANCEL FEE WITHIN 24H: ' + CAST(@CancelFeeWithin24h AS NVARCHAR(10));
...
PRINT N'=== ĐÃ TẠO STORED PROCEDURE: USP_MANAGER_CONFIG_BRANCH ===';
GO

-- ====================================================================
-- Demo Unrepeatable Read (Branch config thay đổi trong lúc lễ tân đang hủy sân)
-- T2 - Quản lý (chạy nhanh, đổi phí hủy lên 70%):
-- Lưu ý cần thay đổi ManagerUserID thành ID của Manager trong hệ thống
SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
BEGIN TRAN;
EXEC USP_MANAGER_CONFIG_BRANCH
    @ManagerUserID = '03FFCB40-2704-4E74-B6F7-CF77BF89FB69',
    @CancelFeeWithin24h = 0.8;
COMMIT;
-- Kết quả: T1 dùng phí cũ (50%) dù sau đó cấu hình đã tăng lên 70% → unrepeatable read/stale config.
GO
